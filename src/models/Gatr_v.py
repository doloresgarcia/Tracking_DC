from os import path
import sys
from src.gatr import GATr, SelfAttentionConfig, MLPConfig
...
class ExampleWrapper(L.LightningModule):  # nn.Module L.LightningModule
    ...
    def forward(self, embedded_inputs):
        ...
        # scalars = torch.zeros((embedded_inputs.shape[0], 1))
    ...
    def training_step(self, batch, batch_idx):
        ...
        embedded_inputs = (
            embed_point(pos_hits_xyz) + embed_scalar(hit_type) + velocities
        )
        embedded_inputs = embedded_inputs.unsqueeze(-2)
        model_output = self(embedded_inputs)
