...
def initialize(t, store_tau,detector_version):
    ...
    if detector_version == 3:
        ...
        # for digitized hits
        pos_wire_x = ROOT.std.vector("float")()
        pos_wire_y = ROOT.std.vector("float")()
        pos_wire_z = ROOT.std.vector("float")()
        wire_stereo_angle = ROOT.std.vector("float")()
        wire_azimuthal_angle = ROOT.std.vector("float")()
        distance = ROOT.std.vector("float")()
        cluster_count = ROOT.std.vector("float")()
        ...
        t.Branch("pos_wire_x", pos_wire_x)
        t.Branch("pos_wire_y", pos_wire_y)
        t.Branch("pos_wire_z", pos_wire_z)
        t.Branch("wire_stereo_angle", wire_stereo_angle)
        t.Branch("wire_azimuthal_angle", wire_azimuthal_angle)
        t.Branch("distance", distance)
        t.Branch("cluster_count", cluster_count)
        ...
        dic = {
            ...
            "gen_status": gen_status,
            "hit_cellID": hit_cellID,
            "pos_wire_x": pos_wire_x,
            "pos_wire_y": pos_wire_y,
            "pos_wire_z": pos_wire_z,
            "wire_stereo_angle": wire_stereo_angle,
            "wire_azimuthal_angle": wire_azimuthal_angle,
            "distance": distance,
            "produced_by_secondary": produced_by_secondary,
            "cluster_count": cluster_count,
            "superLayer": superLayer,
...
def store_hit_col_CDC(
    ...
    if detector_version == 3:
        dc_hits_digi = event.get("DCH_DigiCollection")
        ...
        for num_hit, dc_hit in enumerate(dc_hits):
            ...
            position_digi = dc_hit_digi.getPosition()
            stereoAngle_wire = dc_hit_digi.getWireStereoAngle()
            azimuthalAngle_wire = dc_hit_digi.getWireAzimuthalAngle()
            distance = dc_hit_digi.getDistanceToWire()
            cluster_count = dc_hit_digi.getNClusters()
            dic["pos_wire_x"].push_back(position_digi.x)
            dic["pos_wire_y"].push_back(position_digi.y)
            dic["pos_wire_z"].push_back(position_digi.z)
            dic["wire_stereo_angle"].push_back(stereoAngle_wire)
            dic["wire_azimuthal_angle"].push_back(azimuthalAngle_wire)
            dic["distance"].push_back(distance)
...
def store_hit_col_VTX(
    ...
    if detector_version == 3:
        ...
        vtxD_hits = event.get("VertexEndcapCollection")
        vtxB_hits = event.get("VertexBarrelCollection")
        hit_collections = [vtxD_hits, vtxB_hits]
        
        vtxD_hits_digi = event.get("VertexEndcapCollection_digi")
        vtxB_hits_digi = event.get("VertexBarrelCollection_digi")
        
        hit_collections_digi = [vtxD_hits_digi, vtxB_hits_digi]
        
        list_of_MCs1 = []
        for coll_number, coll in enumerate(hit_collections):
            ii = 0
            
            for vtx_hit_index, vtx_hit in enumerate(coll):
                
                momentum = vtx_hit.getMomentum()
                px = momentum.x
                py = momentum.y
                pz = momentum.z
                
                vtx_hit_digi = hit_collections_digi[coll_number][vtx_hit_index]
                covMatrix = vtx_hit_digi.getCovMatrix()
                cellID = vtx_hit.getCellID()
                EDep = vtx_hit_digi.getEDep()
                time = vtx_hit_digi.getTime()
                pathLength = vtx_hit.getPathLength()
                
                position_digi = vtx_hit_digi.getPosition()
                x = position_digi.x
                y = position_digi.y
                z = position_digi.z
                
                dic["pos_wire_x"].push_back(0)
                dic["pos_wire_y"].push_back(0)
                dic["pos_wire_z"].push_back(0)
                dic["wire_stereo_angle"].push_back(0)
                dic["wire_azimuthal_angle"].push_back(0)
                dic["distance"].push_back(0)
                dic["cluster_count"].push_back(0)
                ...
                dic["hit_pz"].push_back(pz)
                dic["hit_type"].push_back(htype)
                produced_by_secondary = vtx_hit.isProducedBySecondary()
                dic["superLayer"].push_back(0)
                dic["layer"].push_back(0)
                dic["phi"].push_back(0)
                dic["stereo"].push_back(0)
                dic["produced_by_secondary"].push_back(1.0 * produced_by_secondary)
                mcParticle = vtx_hit.getMCParticle()
                object_id = mcParticle.getObjectID()
                genlink0 = object_id.index
                if genlink0 == 74:
                    number_of_hits_p_seatched = number_of_hits_p_seatched + 1
